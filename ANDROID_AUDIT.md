...
- [ ] Replace full-domain realtime reloads with domain-specific listeners, explicit revisions/conflict policy and durable offline outbox/retry. (Domain-specific listeners are complete. Transactions now use an atomic Room v17 outbox, network-constrained WorkManager retry and cold-sync preservation of pending edits/deletes; Auth+Firestore-emulator E2E proves save/delete delivery and recoverable permission errors. Extend the same mechanism to every mutable domain and add explicit revisions/conflict policy.)
...
- [ ] Show operation-level local/pending/synced/error state instead of only a global sync banner. (Transaction history now exposes per-row and aggregate synced/pending/error states with non-color TalkBack labels. Remaining mutable domains pending.)
