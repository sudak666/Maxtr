...
- [ ] Add bulk edit, undo, encrypted backup/restore and reliable import/export recovery. (Bulk transaction selection, bulk category edit/delete and remote-aware undo are implemented and interaction-tested. Portable all-17-table active-profile backup uses AES-256-GCM + PBKDF2, strict schema/column validation, atomic scope-safe restore, and passing crypto/round-trip/tamper tests. Settings UI plus explicitly authorized cloud replacement after restore remain.)
