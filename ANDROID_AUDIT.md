...
- [ ] Add bulk edit, undo, encrypted backup/restore and reliable import/export recovery. (Bulk transaction selection, bulk category edit/delete and remote-aware undo are implemented and interaction-tested. Portable all-17-table active-profile backup uses AES-256-GCM + PBKDF2, strict schema/column validation and atomic scope-safe restore. Owner-only cloud replacement deletes stale remote transactions/documents, restores Room, then durably queues every domain; production-rules Auth+Firestore emulator E2E proves restored delivery and stale-remote deletion. Settings restore UI with preview/explicit confirmation remains.)
