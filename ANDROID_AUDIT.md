...
- [x] Replace `fallbackToDestructiveMigration` with explicit, tested Room migrations (authoritative v13 schema regenerated from historical commit `1f87df7`; connected 13→18 migration preserves data and validates every later schema/index. v14 was an internal transition and never a published database version.)
...
- [ ] Show operation-level local/pending/synced/error state instead of only a global sync banner. (Transaction history exposes per-row and aggregate states; Shopping, Debt and Shifts expose pending/error snapshot state through a shared TalkBack-readable, non-color component. Remaining Finance subdomains pending.)
