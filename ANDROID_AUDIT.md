...
- [ ] Replace full-domain realtime reloads with domain-specific listeners, explicit revisions/conflict policy and durable offline outbox/retry. (Domain-specific listeners are complete. Transactions, Shopping and Debt now use an atomic Room v17 outbox, network-constrained WorkManager retry and cold-sync preservation of pending local work; Auth+Firestore-emulator E2E proves delivery, recoverable permission errors and no cross-profile snapshot leakage after profile switching. Extend it to remaining mutable domains and add explicit revisions/conflict policy.)
...
- [ ] Show operation-level local/pending/synced/error state instead of only a global sync banner. (Transaction history exposes per-row and aggregate states; Shopping and Debt expose pending/error snapshot state through a shared TalkBack-readable, non-color component. Remaining mutable domains pending.)
