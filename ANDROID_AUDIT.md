...
- [ ] Remove the plaintext Monobank token from Firestore; store it only with Android Keystore-backed encryption, migrate/delete legacy remote tokens, and test reconnect/sync/disconnect. (Remote serialization excludes the token; migration writes synchronously before remote deletion; real Keystore encryption/isolation/delete instrumentation passes. A real Auth+Firestore emulator E2E now proves legacy remote-token deletion after Keystore persistence, reconnect without remote secret leakage, statement import/dedup persistence and disconnect deletion under deployed rules. Final production-proxy run with the owner's real Monobank token remains a physical-device gate.)
...
- [x] Replace `fallbackToDestructiveMigration` with explicit, tested Room migrations (authoritative v13 schema regenerated from historical commit `1f87df7`; connected 13→19 migration preserves data and validates every later schema/index. v14 was an internal transition and never a published database version.)
...
- [x] Replace full-domain realtime reloads with domain-specific listeners, explicit revisions/conflict policy and durable offline outbox/retry. (Transactions, including bulk/CSV, every local Finance field, Shopping, Debt and Shifts use the Room v19 outbox plus network-constrained WorkManager. Transactions preserve both concurrent edits and reject stale deletes; every snapshot domain persists its last server revision and retains stale operations as visible conflicts without overwriting newer remote data. Cold sync preserves pending local work. Firebase-emulator E2E covers delivery, concurrent conflicts, permission errors and cross-profile isolation.)
- [x] Make Room data profile-scoped (`ownerUid` + `profileId`) so profiles remain isolated and available offline (Room v16 adds composite scope keys to all 17 tables; every DAO and sync read/update/delete/replace explicitly carries the requested scope; repository Flows rebind on profile changes; legacy v15 rows are adopted once. Connected 13→19, same-ID isolation and production-rules Finance snapshot E2E pass; the latter switches active profile before writing every Finance subdomain and proves no private marker reaches the target remote document.)
- [x] Add structured error types, safe diagnostics/Crashlytics redaction and actionable retry states. (Realtime, cold sync and every durable outbox domain classify network/auth/permission/rate-limit/conflict/data failures and expose localized retry/error state. Diagnostics accept only a closed domain plus sanitized stable code; a source-contract test rejects exception messages, stack traces and Crashlytics recording APIs across production sources. Crashlytics is deliberately absent until explicit opt-in/deletion/redaction exists. Monobank maps HTTP status to bounded localized actions and never renders/logs response or exception text.)
- [x] Show operation-level local/pending/synced/error state instead of only a global sync banner. (Transaction history exposes per-row and aggregate states; Finance snapshots, Shopping, Debt and Shifts expose pending/error state through TalkBack-readable, non-color UI.)
...
- [x] Add bulk edit, undo, encrypted backup/restore and reliable import/export recovery. (Bulk transaction selection, bulk category edit/delete and remote-aware undo are interaction-tested. Portable all-17-table active-profile backup uses AES-256-GCM + PBKDF2, bounded reads, strict schema/column validation and atomic scope-safe restore. Owner-only cloud replacement deletes stale remote transactions/documents, restores Room, then durably queues every domain; production-rules Auth+Firestore emulator E2E proves restored delivery and stale-remote deletion. Settings provides file/password validation, non-mutating row/table preview and separate destructive confirmation; shared profiles cannot access restore.)
...
- [ ] Split oversized coordinators/ViewModels/sheets; move migration-history comments out of production code. (All migration-history narratives are removed and guarded. Settings fell from 1,094 to 800 lines; pure filtering/monthly totals moved out of `FinanceViewModel`, reducing it from 441 to 402. Finance presentation components and `TransactionRow` moved out of `FinanceScreen`, reducing it from 751 to 306 orchestration lines. All boundaries have source-size regression caps. Remaining large screens/coordinator cleanup remains.)
