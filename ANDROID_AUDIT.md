...
- [ ] Replace full-domain realtime reloads with domain-specific listeners, explicit revisions/conflict policy and durable offline outbox/retry. (Domain-specific listeners are complete. Transactions, including bulk and CSV import, every local Finance snapshot field, Shopping, Debt and Shifts use the Room v19 outbox, network-constrained WorkManager retry and cold-sync preservation of pending local work. Transactions preserve both concurrent edits and reject stale deletes. Finance fields use independent persisted revisions and retain stale operations as visible conflicts, preventing cross-field or stale overwrites. Emulator E2E covers delivery, cold-sync safety, conflicts, permission errors and cross-profile isolation. Explicit conflict policies remain for Shopping, Debt and Shifts snapshots.)
...
- [ ] Add structured error types, safe diagnostics/Crashlytics redaction and actionable retry states. (Realtime/cold/outbox sync classifies network/auth/permission/rate-limit/conflict/data failures and routes diagnostics through a closed domain enum plus sanitized stable code only. A source-contract test rejects exception messages, stack traces and Crashlytics recording APIs. Crashlytics is deliberately absent until explicit opt-in/deletion/redaction exists; policy is documented. Localized retry exists for sync and durable-domain errors; remaining direct Finance feature paths pending.)
