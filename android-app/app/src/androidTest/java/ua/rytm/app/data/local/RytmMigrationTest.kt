...
class RytmMigrationTest {
    ...
    @Test
    fun migrate13To19PreservesDataAndCreatesNewSchema() {
        ...
        helper.runMigrationsAndValidate(DB_NAME, 19, true, *RytmMigrations.ALL).use { db ->
            ...
            db.query("SELECT COUNT(*) FROM sync_revisions").use { cursor ->
                assertEquals(true, cursor.moveToFirst())
                assertEquals(0, cursor.getInt(0))
            }
        }
