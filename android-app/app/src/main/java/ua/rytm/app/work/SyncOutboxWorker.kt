...
class SyncOutboxWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    override suspend fun doWork(): Result {
        ...
        return runCatching {
            ...
            val shiftsDrained = app.shiftsSyncRepository.drainOutbox()
            val financeDrained = app.financeSnapshotOutboxRepository.drainOutbox()
            transactionsDrained && shoppingDrained && debtDrained && shiftsDrained && financeDrained
        }.fold(
