...
**Якщо код SMS не надсилається взагалі, навіть на новий номер** — консоль
браузера показує `POST .../accounts:sendVerificationCode 503 (Service
Unavailable)` і `FirebaseError: Firebase: Error (auth/error-code:-39)`.
Код -39 = `TOO_MANY_ATTEMPTS_TRY_LATER`. На перший погляд це виглядає як
проблема конфігурації проєкту, бо падає навіть на щойно введеному номері,
якого раніше не пробували — але Firebase кидає цю саму помилку і при
**рейт-лімітах по IP/пристрою** (не тільки по конкретному номеру): ліміт
десь ~150 запитів верифікації на IP/годину і ~50 SMS/хвилину на IP. Якщо
тестувати кілька номерів поспіль з одного браузера за короткий час — саме
це і трапляється.

Перевірено напряму через Identity Toolkit Admin API для `maxtr-c238f`
(GET `.../admin/v2/projects/maxtr-c238f/config`), щоб виключити інші
причини:
- `recaptchaEnforcementState` для `PHONE_PROVIDER` — `UNSPECIFIED`
  (reCAPTCHA Enterprise **не** вимагається для цього проєкту, тож
  відсутність Enterprise-ключа тут ні до чого);
- білінг активний (Blaze), `smsRegionConfig.allowedRegions` включає `UA`.

Тобто це не конфігураційна прогалина, а вбудований анти-абуз захист
Firebase, що спрацював через часті повторні спроби під час тестування.
Рішення:
1. Почекати (зазвичай знімається за годину-дві; за сильнішого спрацювання
   — довше) і не тестувати кілька номерів поспіль.
2. Для розробки/тестування додати **тестові номери телефону**:
   Firebase Console → **Authentication → Sign-in method → Phone →
   Phone numbers for testing** — фейковий номер + фіксований код, SMS
   реально не надсилається і під цей рейт-ліміт не підпадає.
