...
До цих змін усі дані (зміни, фінанси, борги) лежали в одному документі
`max_tracker/*` без власника. Ці правила закривають доступ до нього
(`allow read, write: if false`). Якщо там є дані, які потрібно зберегти —
експортуй їх вручну через Firebase Console, поки не задеплоїв нові правила.
