...
export function setupModalAccessibility(){
  ...
  overlays.forEach(overlay=>{
    ...
    const card=/** @type {HTMLElement | null} */ (overlay.querySelector('.modal-card, .dlg-card'));
  ...
  const observer=new MutationObserver(mutations=>{
    mutations.forEach(m=>{
      ...
      if(!(target instanceof Element) || !target.classList.contains('modal-overlay')) return;
      const overlay=/** @type {HTMLElement & {_returnFocusTo?: HTMLElement | null}} */ (target);
      ...
      if(isOpen && stackIdx===-1){
        ...
        overlay._returnFocusTo=(returnFocusTo && returnFocusTo!==document.body)?/** @type {HTMLElement} */ (returnFocusTo):null;
        ...
        const card=/** @type {HTMLElement | null} */ (overlay.querySelector('.modal-card, .dlg-card'));
        if(card) setTimeout(()=>{ (/** @type {HTMLElement} */ (focusableIn(card)[0]||card)).focus(); },0);
...
export function renderPrivacyCacheUI(){
  const cb=/** @type {HTMLInputElement | null} */ (document.getElementById('privacy-cache-checkbox'));
...
function renderRatesSourceUI(){
  document.querySelectorAll('#rates-source-filter .filter-chip').forEach(c=>c.classList.toggle('active', /** @type {HTMLElement} */ (c).dataset.source===AppState.ratesSource));
...
export function populateFxConverterSelects(){
  const from=/** @type {HTMLSelectElement | null} */ (document.getElementById('fx-conv-from')), to=/** @type {HTMLSelectElement | null} */ (document.getElementById('fx-conv-to'));
...
export function renderFxConverter(){
  const amountEl=/** @type {HTMLInputElement | null} */ (document.getElementById('fx-conv-amount'));
  const fromEl=/** @type {HTMLSelectElement | null} */ (document.getElementById('fx-conv-from')), toEl=/** @type {HTMLSelectElement | null} */ (document.getElementById('fx-conv-to'));
...
function renderCategoriesList(){
  ...
  list.forEach((name,idx)=>{
    ...
    if(open){
      const addBtn=/** @type {HTMLElement | null} */ (row.querySelector('.btn-ghost'));
...
document.addEventListener('click', (e)=>{
  ...
  if(target.classList.contains('modal-overlay') || target.closest('[data-close-modal]')) closeManagers();
}, true);
...
/** @type {any} */ (window).openWalletsManager = openWalletsManager;
/** @type {any} */ (window).openWidgetsManager = openWidgetsManager;
renderPrivacyCacheUI();
