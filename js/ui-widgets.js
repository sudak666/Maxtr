...
/**
 * @param {{icon?: string, title?: string, desc?: string, action?: string, actionName?: string}} opts
 * @returns {string}
 */
...
export const filterSettings = function(q){
  ...
  sections.forEach(section=>{
    const groupHit=AppState.activeSettingsGroup==='all' || /** @type {HTMLElement} */ (section).dataset.settingsGroup===AppState.activeSettingsGroup;
...
const setSettingsGroup = function(group){
  ...
  document.querySelectorAll('.settings-group-chip').forEach(chip=>{
    chip.classList.toggle('active', /** @type {HTMLElement} */ (chip).dataset.group===AppState.activeSettingsGroup);
  });
  syncClickableA11yState(document.getElementById('tab-settings'));
  filterSettings(/** @type {HTMLInputElement | null} */ (document.getElementById('settings-search-input'))?.value||'');
};
...
export function setupCollapsibleFinanceSections(){
  ['budgets-section','goals-section','finance-chart-section'].forEach(id=>{
    ...
    const title=/** @type {HTMLElement | null} */ (section&&section.querySelector('.chart-title'));
...
export function setupAccessibleClickableDivs(root){
  ...
  (root||document).querySelectorAll('[data-action]').forEach(elRaw=>{
    const el=/** @type {HTMLElement} */ (elRaw);
...
export function enhanceAllSelects(){
  ...
  document.addEventListener('scroll',e=>{
    ...
    if(t.closest && t.closest('.cs-panel,.dp-panel')) return;
    closeAllPanels();
  },true);
