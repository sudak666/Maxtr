...
const confirmAddGoal = function(){
  ...
  const walletSel=/** @type {HTMLSelectElement | null} */ (document.getElementById('goal-new-wallet'));
  const walletId=walletSel&&walletSel.value?walletSel.value:AppState.wallets[0].id;
  const targetAmount=parseFloat(/** @type {HTMLInputElement} */ (document.getElementById('goal-new-amount')).value)||0;
  const targetDate=/** @type {HTMLInputElement} */ (document.getElementById('goal-new-date')).value||'';
...
export function renderGoals(){
  ...
  AppState.goals.forEach(g=>{
    ...
    row.innerHTML=`
      ...
      ${done?`<div style="font-size:13px;color:var(--green2);font-weight:700;margin-top:3px">${tr('goals_reached')}</div>`:''}
    `;
    box.appendChild(row);
  });
}
...
const handleAvatarUpload = function(event){
  ...
  reader.onload=e=>{
    ...
    img.src=/** @type {string} */ (/** @type {FileReader} */ (e.target).result);
  };
...
document.addEventListener('focusout', e=>{
  const el=/** @type {HTMLElement | null} */ (/** @type {Element} */ (e.target).closest('[data-blur-action]'));
