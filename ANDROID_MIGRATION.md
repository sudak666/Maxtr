...
Кроки 1-32 повністю закриті й реально перевірені наскрізь (включно зі спільними профілями — round-trip підтверджено PR #394). Далі: гранулярні editor/viewer ролі для спільних профілів (розширення кроку 32, `listSharedMembers()`/`setMemberRole()` з PWA) — чи мова (i18n, блокована міграцією на string resources)? Запитай власника акаунта перед стартом.
