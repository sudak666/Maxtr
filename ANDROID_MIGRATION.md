...
**Статус**: Кроки 1-23 зроблені й **реально перевірені наскрізь** (див. розділи нижче по порядку — кожен має "що зроблено"/"чесно not done"). Working screens: Фінанси (тепер з реальними підкатегоріями), Гаманці/Категорії(+підкатегорії)/**Бюджети (крок 23)**/Типи змін, Покупки, Графік змін, Розрахунки/Debt (з payoff-forecast графіком), Налаштування → Вигляд+Акаунт+**Безпека (PIN+біометрія, крок 17)**, реальний вхід через Google. **Firestore cold-sync (одноразовий, при вході, не continuous)** покриває дев'ять доменів: гаманці, типи змін, категорії, підкатегорії, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, **бюджети (крок 23)**. categoryIcons/теги/повторювані операції/autoFillSchedule — жодне ще не має Room-моделі/UI на Android. Не почато: мова (uk/en), Push, профілі, continuous sync для будь-якого домену.

**Наступний крок (не вирішено, запитай або вирішуй сам)**: categoryIcons (чисто оздоблення, нижчий пріоритет), наступний новий домен з нуля (tags/recurring — обидва складніші за бюджети), чи решта Settings (мова/Push/профілі).
...
Далі: categoryIcons (чисто оздоблення), наступний новий домен з нуля (tags/recurring — обидва складніші за бюджети: tags потребує кольорового пікера, recurring — повної форми транзакції + логіки наступної дати), чи решта Settings (мова/Push/профілі)? Працюю строго покроково.
