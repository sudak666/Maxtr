...
**Статус (оновлено після кроку 37, 2026-08-21)**: Кроки 1-37 зроблені й **реально перевірені наскрізь**, включно зі спільними профілями (крок 32), гранулярними editor/viewer ролями (крок 33), видаленням акаунту (крок 35), **release-інфраструктурою для Play Store + реальним Google-входом на фізичному пристрої (крок 36)** і **виправленням 3 нероздіні кнопок Фінансів + Tools/Goals з нуля (крок 37)** — застосунок готовий до реального вивантаження в Play Console. Working screens: Фінанси (підкатегорії, теги з бейджами в списку, регулярні платежі з матеріалізацією + іконкою категорії, іконки категорій на 4 поверхнях + ручний пікер, **Інструменти**-sheet з аналітикою/курсами/конвертером/6-місячним графіком, **Цілі**-manager, Бюджети підключено до кнопки), Гаманці/Категорії(+підкатегорії+іконки)/Бюджети/Теги/Регулярні платежі/Типи змін, Покупки, Графік змін, Розрахунки/Debt, Налаштування → Вигляд+Акаунт(вихід+видалення)+Безпека(PIN+біометрія)+Сповіщення(Push-токен + гранулярні типи)+Профілі(власні + спільні invite/join/leave/ролі), **реальний вхід через Google підтверджено на фізичному пристрої** (крок 36). **Firestore cold-sync (одноразовий, при вході або при перемиканні профілю, не continuous)** покриває **13 доменів** (крок 37 додав Goals і CurrencyRates), усі profileId-aware через `ProfileDocNames.kt`/`ProfileSyncCoordinator`, resolvable проти чужого `ownerUid`-дерева для спільного профілю: гаманці, типи змін, категорії, підкатегорії, categoryIcons, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, теги, регулярні платежі, цілі, курси валют. autoFillSchedule — єдине, що ще не має Room-моделі/UI. Не почато: мова (uk/en); візуальна паритетність з PWA (діагностовано, не реалізовано — див. крок 37).

**Наступний крок**: власник акаунта вивантажує вже зібраний `.aab` у Play Console → Закрите тестування (той самий трек, той самий package `ua.rytm.app`, замінює старий TWA-білд) — дія поза межами агента. Технічно наступне — 3 відкриті пункти без визначеного пріоритету, запитай власника акаунта перед стартом: (1) візуальна паритетність з PWA (власник акаунта: "виглядає дуже хуже чем пва" — діагностовано порівняльним аудитом реальних скріншотів телефону, конкретні PWA CSS-джерела в `index.html`: `.quick-action`/`.quick-action-icon` — кругла форма іконок quick-action-рядка, зараз в Android квадратна; `.finance-action-primary`/`.fin-fab.finance-fab` — зелений градієнт на основній дії/FAB Фінансів, зараз в Android суцільний фіолетовий; `.fin-mini-stat.income`/`.expense` — tinted зелений/червоний фон під дохід/витрата міні-статами, зараз в Android нейтральний; `.hero-balance`/`.hero-metric`'s `--surface-hero`/`--shadow-raised` — градієнт+glow на hero-картці балансу, зараз в Android плоска картка без glow — жодного з цього ще не перенесено в Compose); (2) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (3) continuous sync для всіх 13 доменів (зараз лише cold-sync при вході).
...
Кроки 1-37 повністю закриті й реально перевірені наскрізь. Відкриті пункти: (1) раніше діагностована, але не реалізована візуальна робота — застосунок виглядає "гірше за ПВА" за словами власника акаунта (кругові quick-action іконки, зелений FAB на Фінансах, tinted income/expense картки, glow/gradient hero-card — точні CSS-токени з `index.html` вже задокументовані в історії сесії, лишилось перенести їх у Compose); (2) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (3) continuous (не лише cold-sync) синхронізація для всіх 13 доменів. Запитай власника акаунта перед стартом, який пріоритет.
