...
Кроки 1-42 зроблені; крок 42 **компілюється, але не перевірений живим прогоном** (див. вище — інфраструктурна перешкода цієї сесії, не проблема коду). Відкриті пункти: (1) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (2) continuous (не лише cold-sync) синхронізація для всіх доменів; (3) **живий емулятор-прогін кроку 42**, щойно з'явиться вільна пам'ять на хості. Запитай власника акаунта перед стартом, який пріоритет.
