...
**Статус**: Кроки 1-22 зроблені й **реально перевірені наскрізь** (див. розділи нижче по порядку — кожен має "що зроблено"/"чесно not done"). Working screens: Фінанси (тепер з реальними підкатегоріями), Гаманці/Категорії(+підкатегорії)/Типи змін, Покупки, Графік змін, Розрахунки/Debt (з payoff-forecast графіком), Налаштування → Вигляд+Акаунт+**Безпека (PIN+біометрія, крок 17)**, реальний вхід через Google. **Firestore cold-sync (одноразовий, при вході, не continuous)** покриває вісім доменів: гаманці, типи змін, категорії, **підкатегорії (крок 22)**, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря. categoryIcons/бюджети/теги/повторювані операції/autoFillSchedule — жодне ще не синхронізується (жодне з них не має Room-моделі/UI на Android взагалі). Не почато: мова (uk/en), Push, профілі, continuous sync для будь-якого домену.

**Наступний крок (не вирішено, запитай або вирішуй сам)**: categoryIcons (чисто оздоблення, нижчий пріоритет), новий домен з нуля (budgets/tags/recurring — потребують нових Room-моделей і UI, більший крок), чи решта Settings (мова/Push/профілі).
...
Далі: categoryIcons (третя частина categories-кластера, чисто оздоблення), новий домен з нуля (budgets/tags/recurring — потребують нових Room-моделей і UI, більший крок), чи решта Settings (мова/Push/профілі)? Працюю строго покроково.
