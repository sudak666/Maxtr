...
**Статус**: Кроки 1-24 зроблені й **реально перевірені наскрізь** (див. розділи нижче по порядку — кожен має "що зроблено"/"чесно not done"). Working screens: Фінанси (з реальними підкатегоріями і **вибором тегів у формі, крок 24**), Гаманці/Категорії(+підкатегорії)/Бюджети/**Теги (крок 24)**/Типи змін, Покупки, Графік змін, Розрахунки/Debt (з payoff-forecast графіком), Налаштування → Вигляд+Акаунт+**Безпека (PIN+біометрія, крок 17)**, реальний вхід через Google. **Firestore cold-sync (одноразовий, при вході, не continuous)** покриває десять доменів: гаманці, типи змін, категорії, підкатегорії, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, **теги (крок 24)**. categoryIcons/повторювані операції/autoFillSchedule — жодне ще не має Room-моделі/UI на Android. Не почато: мова (uk/en), Push, профілі, continuous sync для будь-якого домену.

**Наступний крок (не вирішено, запитай або вирішуй сам)**: categoryIcons (чисто оздоблення), показ тегів у рядку списку транзакцій (візуальне доопрацювання), recurring (найскладніший з нових доменів), чи решта Settings (мова/Push/профілі).
...
16. **⚠️ Незакомічена робота, нез'ясована гілка**: весь `android-app/` і цей файл сидять НЕЗАКОМІЧЕНИМИ прямо в робочому дереві `main` вже багато сесій поспіль (перевір `git status`). `claude/native-applications-638ap0`, яку CLAUDE.md називає "гілкою, що лишається незлитою" для native Android — це СТАРА покинута спроба (`ua.zminka.app`/`./android/`), НЕ поточна робота (`ua.rytm.app`/`./android-app`). Перш ніж комітити цю роботу для реального, треба вирішити: перевикористати стару назву гілки чи взяти нову, і оновити виняток у CLAUDE.md. Не комітити без явного рішення власника акаунта.
...
Далі: categoryIcons (чисто оздоблення), показ тегів у рядку списку транзакцій (візуальне доопрацювання кроку 24), recurring (найскладніший з домену "з нуля" — повна форма транзакції + логіка наступної дати), чи решта Settings (мова/Push/профілі)? Працюю строго покроково.
