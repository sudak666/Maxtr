...
**Статус (оновлено після кроку 38, 2026-08-21)**: Кроки 1-38 зроблені й **реально перевірені наскрізь**, включно зі спільними профілями (крок 32), гранулярними editor/viewer ролями (крок 33), видаленням акаунту (крок 35), **release-інфраструктурою для Play Store + реальним Google-входом на фізичному пристрої (крок 36)**, **виправленням 3 нероздіні кнопок Фінансів + Tools/Goals з нуля (крок 37)** і **візуальною паритетністю Фінансів з PWA — hero-card градієнт+glow, circular quick-action бейджі, tinted income/expense картки, зелений FAB (крок 38)** — застосунок готовий до реального вивантаження в Play Console. Working screens: Фінанси (підкатегорії, теги з бейджами в списку, регулярні платежі з матеріалізацією + іконкою категорії, іконки категорій на 4 поверхнях + ручний пікер, **Інструменти**-sheet з аналітикою/курсами/конвертером/6-місячним графіком, **Цілі**-manager, Бюджети підключено до кнопки), Гаманці/Категорії(+підкатегорії+іконки)/Бюджети/Теги/Регулярні платежі/Типи змін, Покупки, Графік змін, Розрахунки/Debt, Налаштування → Вигляд+Акаунт(вихід+видалення)+Безпека(PIN+біометрія)+Сповіщення(Push-токен + гранулярні типи)+Профілі(власні + спільні invite/join/leave/ролі), **реальний вхід через Google підтверджено на фізичному пристрої** (крок 36). **Firestore cold-sync (одноразовий, при вході або при перемиканні профілю, не continuous)** покриває **13 доменів** (крок 37 додав Goals і CurrencyRates), усі profileId-aware через `ProfileDocNames.kt`/`ProfileSyncCoordinator`, resolvable проти чужого `ownerUid`-дерева для спільного профілю: гаманці, типи змін, категорії, підкатегорії, categoryIcons, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, теги, регулярні платежі, цілі, курси валют. autoFillSchedule — єдине, що ще не має Room-моделі/UI. Не почато: мова (uk/en); continuous sync; візуальна звірка інших екранів (Графік змін/Розрахунки/Покупки/Налаштування) — лише Фінанси перевірено детально в кроці 38.

**Наступний крок**: власник акаунта вивантажує вже зібраний `.aab` у Play Console → Закрите тестування (той самий трек, той самий package `ua.rytm.app`, замінює старий TWA-білд) — дія поза межами агента. Технічно наступне — 3 відкриті пункти без визначеного пріоритету, запитай власника акаунта перед стартом: (1) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (2) continuous sync для всіх 13 доменів (зараз лише cold-sync при вході); (3) подальша візуальна звірка інших екранів (Графік змін/Розрахунки/Покупки/Налаштування) проти PWA CSS — крок 38 закрив лише Фінанси (`.quick-action`/`.fin-fab.finance-fab`/`.fin-mini-stat`/`.hero-balance`), інші екрани не перевірялись так само детально.
...
Кроки 1-38 повністю закриті й реально перевірені наскрізь. Відкриті пункти: (1) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (2) continuous (не лише cold-sync) синхронізація для всіх 13 доменів; (3) можлива подальша візуальна звірка інших екранів (Графік змін/Розрахунки/Покупки/Налаштування) проти PWA CSS — не перевірялась так само детально, як Фінанси в кроці 38. Запитай власника акаунта перед стартом, який пріоритет.
