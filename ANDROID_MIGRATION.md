...
**Статус (оновлено після кроку 36, 2026-08-21)**: Кроки 1-36 зроблені й **реально перевірені наскрізь**, включно зі спільними профілями (крок 32), гранулярними editor/viewer ролями (крок 33), видаленням акаунту (крок 35) і **release-інфраструктурою для Play Store + реальним Google-входом на фізичному пристрої (крок 36)** — застосунок готовий до реального вивантаження в Play Console. Working screens: Фінанси (підкатегорії, теги з бейджами в списку, регулярні платежі з матеріалізацією + іконкою категорії, іконки категорій на 4 поверхнях + ручний пікер), Гаманці/Категорії(+підкатегорії+іконки)/Бюджети/Теги/Регулярні платежі/Типи змін, Покупки, Графік змін, Розрахунки/Debt, Налаштування → Вигляд+Акаунт(вихід+видалення)+Безпека(PIN+біометрія)+Сповіщення(Push-токен + гранулярні типи)+Профілі(власні + спільні invite/join/leave/ролі), **реальний вхід через Google підтверджено на фізичному пристрої** (крок 36 — раніше перевірявся лише анонімний тестовий вхід через емулятор). **Firestore cold-sync (одноразовий, при вході або при перемиканні профілю, не continuous)** покриває **11 доменів**, усі profileId-aware через `ProfileDocNames.kt`/`ProfileSyncCoordinator`, resolvable проти чужого `ownerUid`-дерева для спільного профілю: гаманці, типи змін, категорії, підкатегорії, categoryIcons, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, теги, регулярні платежі. autoFillSchedule — єдине, що ще не має Room-моделі/UI. Не почато: мова (uk/en).

**Наступний крок**: власник акаунта вивантажує вже зібраний `.aab` у Play Console → Закрите тестування (той самий трек, той самий package `ua.rytm.app`, замінює старий TWA-білд) — дія поза межами агента. Технічно наступне: мова (i18n, uk/en) — заблокована реальним prerequisite: усі Kotlin/Compose-файли хардкодять українські рядки напряму, потрібна повна міграція в string resources перед перемикачем — окремий, великий, механічний рефакторинг-крок. Запитай власника акаунта перед стартом.
...
Кроки 1-35 повністю закриті й реально перевірені наскрізь. Далі: мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources. Запитай власника акаунта перед стартом, чи є інший пріоритет.
