...
**Статус (оновлено після кроку 35, 2026-08-21)**: Кроки 1-35 зроблені й **реально перевірені наскрізь**, включно зі спільними профілями (invite/join/switch/leave round-trip — крок 32), гранулярними editor/viewer ролями (крок 33) і видаленням акаунту (крок 35, обране автономно за PM/UX-обґрунтуванням: Google Play вимагає account-deletion для застосунків з можливістю створення акаунту, а Play Console-заявка власника акаунта саме зараз очікує верифікації). Working screens: Фінанси (підкатегорії, теги з бейджами в списку, регулярні платежі з матеріалізацією + іконкою категорії, іконки категорій на 4 поверхнях + ручний пікер), Гаманці/Категорії(+підкатегорії+іконки)/Бюджети/Теги/Регулярні платежі/Типи змін, Покупки, Графік змін, Розрахунки/Debt, Налаштування → Вигляд+Акаунт(вихід+**видалення — крок 35**)+Безпека(PIN+біометрія)+Сповіщення(Push-токен + гранулярні типи)+Профілі(власні + спільні invite/join/leave/ролі), реальний вхід через Google. **Firestore cold-sync (одноразовий, при вході або при перемиканні профілю, не continuous)** покриває **11 доменів**, усі profileId-aware через `ProfileDocNames.kt`/`ProfileSyncCoordinator`, resolvable проти чужого `ownerUid`-дерева для спільного профілю: гаманці, типи змін, категорії, підкатегорії, categoryIcons, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, теги, регулярні платежі. autoFillSchedule — єдине, що ще не має Room-моделі/UI. Не почато: мова (uk/en).

**Наступний крок**: мова (i18n, uk/en) — заблокована реальним prerequisite: усі Kotlin/Compose-файли хардкодять українські рядки напряму. Потрібна повна міграція сотень рядків у string resources ПЕРЕД тим, як можна додати перемикач — окремий, великий, механічний рефакторинг-крок, не "фіча". Запитай власника акаунта перед стартом (обсяг і черга робіт вартують окремого обговорення), чи є інший пріоритет спершу (наприклад веб-шлях видалення акаунту, який Google Play теж вимагає окремо від застосунку — див. крок 35's "Свідомо не в цьому кроці").
