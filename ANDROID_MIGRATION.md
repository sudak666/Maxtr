...
**Статус**: Кроки 1-25 зроблені й **реально перевірені наскрізь** (див. розділи нижче по порядку — кожен має "що зроблено"/"чесно not done"). Working screens: Фінанси (з реальними підкатегоріями, вибором тегів у формі й **тег-бейджами в списку, крок 25**), Гаманці/Категорії(+підкатегорії)/Бюджети/Теги/Типи змін, Покупки, Графік змін, Розрахунки/Debt (з payoff-forecast графіком), Налаштування → Вигляд+Акаунт+**Безпека (PIN+біометрія, крок 17)**, реальний вхід через Google. **Firestore cold-sync (одноразовий, при вході, не continuous)** покриває десять доменів: гаманці, типи змін, категорії, підкатегорії, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, **теги (крок 24)**. categoryIcons/повторювані операції/autoFillSchedule — жодне ще не має Room-моделі/UI на Android. Не почато: мова (uk/en), Push, профілі, continuous sync для будь-якого домену.

**Наступний крок (не вирішено, запитай або вирішуй сам)**:
- **categoryIcons** — найдешевший: третя частина categories-кластера (income/expense/subcategories вже зроблені), суто оздоблення, `finance.categoryIcons` поле, той самий sync-патерн.
- **recurring** — найцінніший, але найдорожчий: повторювані операції з нуля — новий Room-домен + повноцінна форма (майже як `TransactionFormSheet`) + логіка обчислення наступної дати (`js/settings-managers.js`'s `computeNextDate()`) + сам механізм "застосувати, коли настав час" (у PWA це робиться при `fbLoadNow()`). Реалістично кілька кроків, не один.
- **Settings-решта** (мова uk/en, Push, профілі) — кожен свій великий блокер (мова: міграція сотень хардкоджених рядків у `strings.xml`; Push: FCM registration + sweep-логіка; профілі: switching UI) — жоден не "просто ще один домен".
Автономний вибір минулої сесії (кроки 20-25) — категорії/бюджети/теги — вичерпано; далі кожен варіант вимагає реального рішення про обсяг, тому запитай власника акаунта, якщо є сумнів.
...
Далі: профілі (перемикання/спільний доступ) — найбільша з лишків, справжній багатокроковий епік (кожен sync-репозиторій потребує `@profileId`-суфіксації + скидання/перезавантаження Room на switch), тож наступна сесія має розпочати з окремого планування обсягу, а не одразу кодити. Мова (i18n) лишається заблокованою тим самим prerequisite (міграція на string resources), що й раніше.
