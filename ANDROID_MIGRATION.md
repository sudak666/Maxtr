...
**Статус (оновлено після кроку 31, 2026-08-21)**: Кроки 1-31 зроблені й **реально перевірені наскрізь** (див. розділи нижче по порядку — кожен має "що зроблено"/"чесно not done"). Working screens: Фінанси (підкатегорії, теги з бейджами в списку, **регулярні платежі з матеріалізацією — крок 26**, іконки категорій на 3 поверхнях + **ручний пікер — крок 31**), Гаманці/Категорії(+підкатегорії+іконки)/Бюджети/Теги/Регулярні платежі/Типи змін, Покупки, Графік змін, Розрахунки/Debt, Налаштування → Вигляд+Акаунт+Безпека(PIN+біометрія)+**Сповіщення (Push-токен + гранулярні типи — кроки 27-28)**+**Профілі (власні, перемикання — крок 30)**, реальний вхід через Google. **Firestore cold-sync (одноразовий, при вході або при перемиканні профілю, не continuous)** покриває **11 доменів**, усі profileId-aware через `ProfileDocNames.kt`/`ProfileSyncCoordinator`: гаманці, типи змін, категорії, підкатегорії, **categoryIcons (крок 31)**, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, теги, **регулярні платежі (крок 26)**. autoFillSchedule — єдине, що ще не має Room-моделі/UI. Не почато: мова (uk/en), спільні профілі (invite/join/leave/ролі), видалення акаунту.

**Наступний крок (не вирішено, запитай або вирішуй сам)** — обидва лишки великі, жоден не "просто ще один домен", кожен вимагає окремого планування обсягу на початку сесії (не одразу кодити):
- **Спільні профілі** (invite/join/leave/editor-viewer ролі) — розширення вже готового кроку 30 (власні профілі). Складність: `shared_members` документ, invite-код-редемпшн (двоетапний, rules-enforced — див. CLAUDE.md's "Shared profiles" секцію), доступ до чужого `uid`-дерева (не власного акаунту) для власника-суфіксованих документів. `ProfilesRepository`/`ProfileSyncCoordinator` з кроку 30 уже мають "лише власні профілі" фільтр — розширення означає прибрати цей фільтр і додати весь invite/ownerUid-шар.
- **Мова (i18n, uk/en)** — заблокована реальним prerequisite: усі 41 Kotlin/Compose-файл хардкодить українські рядки напряму (жодного `strings.xml`-виклику, крім nav-labels). Потрібна повна міграція сотень рядків у string resources ПЕРЕД тим, як можна додати перемикач — це окремий, великий, механічний рефакторинг-крок, не "фіча".
- Дрібніший, контрольований варіант, якщо потрібен низькоризиковий крок: підключити `CategoryIconBadge` до `RecurringManagerSheet`'s згорнутого рядка (розкрита, не зроблена дрібниця з кроків 29/31) — кілька рядків коду.
Автономний вибір минулих сесій (кроки 26-31) — Push/профілі(власні)/categoryIcons-пікер — вичерпаний; варто запитати власника акаунта перед стартом одного з двох великих лишків.
...
Далі: спільні профілі (invite/join/leave/ролі editor/viewer) — окрема, значно більша підсистема, чи мова (i18n, блокована міграцією на string resources)? Працюю строго покроково.
