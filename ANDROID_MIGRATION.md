...
**Статус (оновлено після кроку 39, 2026-08-21)**: Кроки 1-39 зроблені й **реально перевірені наскрізь**, включно зі спільними профілями (крок 32), гранулярними editor/viewer ролями (крок 33), видаленням акаунту (крок 35), **release-інфраструктурою для Play Store + реальним Google-входом на фізичному пристрої (крок 36)**, **виправленням 3 нероздіні кнопок Фінансів + Tools/Goals з нуля (крок 37)**, **візуальною паритетністю Фінансів з PWA (крок 38)** і **повною паритетністю "Графік змін" — quick-fill/автозаповнення/6-місячний графік + візуальна звірка (крок 39)** — застосунок готовий до реального вивантаження в Play Console. Working screens: Фінанси (підкатегорії, теги з бейджами в списку, регулярні платежі з матеріалізацією + іконкою категорії, іконки категорій на 4 поверхнях + ручний пікер, **Інструменти**-sheet з аналітикою/курсами/конвертером/6-місячним графіком, **Цілі**-manager, Бюджети підключено до кнопки), Гаманці/Категорії(+підкатегорії+іконки)/Бюджети/Теги/Регулярні платежі/Типи змін, Покупки, **Графік змін (тепер з quick-fill/автозаповненням/6-місячним графіком, крок 39)**, Розрахунки/Debt, Налаштування → Вигляд+Акаунт(вихід+видалення)+Безпека(PIN+біометрія)+Сповіщення(Push-токен + гранулярні типи)+Профілі(власні + спільні invite/join/leave/ролі), **реальний вхід через Google підтверджено на фізичному пристрої** (крок 36). **Firestore cold-sync (одноразовий, при вході або при перемиканні профілю, не continuous)** покриває **13 доменів + autoFillSchedule (крок 39)**, усі profileId-aware через `ProfileDocNames.kt`/`ProfileSyncCoordinator`, resolvable проти чужого `ownerUid`-дерева для спільного профілю: гаманці, типи змін, категорії, підкатегорії, categoryIcons, транзакції, покупки, розрахунки/debt, призначення змін на дні календаря, бюджети, теги, регулярні платежі, цілі, курси валют, автозаповнення розкладу змін. Не почато: мова (uk/en); continuous sync; візуальна+функціональна звірка Розрахунків/Покупок/Налаштувань проти PWA — лише Фінанси (крок 38) і Графік змін (крок 39) перевірено детально.

**Наступний крок**: власник акаунта вивантажує вже зібраний `.aab` у Play Console → Закрите тестування (той самий трек, той самий package `ua.rytm.app`, замінює старий TWA-білд) — дія поза межами агента. Технічно наступне — 3 відкриті пункти без визначеного пріоритету, запитай власника акаунта перед стартом: (1) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (2) continuous sync для всіх доменів (зараз лише cold-sync при вході); (3) подальша візуальна+функціональна звірка інших екранів (Розрахунки/Покупки/Налаштування) проти PWA — кроки 38-39 закрили Фінанси й Графік змін, інші екрани не перевірялись так само детально.
...
Кроки 1-39 повністю закриті й реально перевірені наскрізь. Відкриті пункти: (1) мова (i18n, uk/en) — великий механічний рефакторинг, блокований міграцією сотень хардкоджених рядків у string resources; (2) continuous (не лише cold-sync) синхронізація для всіх 13 доменів; (3) можлива подальша візуальна+функціональна звірка інших екранів (Розрахунки/Покупки/Налаштування) проти PWA — не перевірялась так само детально, як Фінанси (крок 38) і Графік змін (крок 39). Запитай власника акаунта перед стартом, який пріоритет.
